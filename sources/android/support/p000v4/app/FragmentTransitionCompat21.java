...
class FragmentTransitionCompat21 {
    ...
    public static void setNameOverridesOptimized(View sceneRoot, ArrayList<View> sharedElementsOut, ArrayList<View> sharedElementsIn, ArrayList<String> inNames, Map<String, String> nameOverrides) {
        ...
        for (int i = 0; i < numSharedElements; i++) {
            ...
            String name2 = view.getTransitionName();
            outNames.add(name2);
            if (name2 != null) {
                view.setTransitionName((String) null);
                String inName = nameOverrides.get(name2);
                int j = 0;
                while (true) {
                    if (j >= numSharedElements) {
                        break;
                    } else if (inName.equals(inNames.get(j))) {
                        sharedElementsIn.get(j).setTransitionName(name2);
                        break;
                    } else {
                        j++;
                    }
        ...
        OneShotPreDrawListener.add(sceneRoot, new Runnable() {
            public void run() {
                ...
                for (int i = 0; i < numSharedElements; i++) {
                    ...
                    String name2 = view.getTransitionName();
                    if (name2 != null) {
                        view.setTransitionName(FragmentTransitionCompat21.findKeyForValue(nameOverrides, name2));
                    }
                }
            }
        });
