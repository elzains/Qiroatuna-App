...
class FragmentTransition {
    ...
    private static ArrayMap<String, View> captureOutSharedElements(ArrayMap<String, String> nameOverrides, Object sharedElementTransition, FragmentContainerTransition fragments) {
        ...
        if (sharedElementCallback != null) {
            ...
            for (int i = names.size() - 1; i >= 0; i--) {
                ...
                View view = outSharedElements.get(name2);
                if (view == null) {
                    nameOverrides.remove(name2);
                } else if (!name2.equals(ViewCompat.getTransitionName(view))) {
                    nameOverrides.put(ViewCompat.getTransitionName(view), nameOverrides.remove(name2));
                }
            }
    ...
    public static ArrayMap<String, View> captureInSharedElements(ArrayMap<String, String> nameOverrides, Object sharedElementTransition, FragmentContainerTransition fragments) {
        ...
        if (sharedElementCallback != null) {
            ...
            for (int i = names.size() - 1; i >= 0; i--) {
                String name2 = names.get(i);
                View view = inSharedElements.get(name2);
                if (view == null) {
                    String key2 = findKeyForValue(nameOverrides, name2);
                    if (key2 != null) {
                        nameOverrides.remove(key2);
                    }
                } else if (!name2.equals(ViewCompat.getTransitionName(view)) && (key = findKeyForValue(nameOverrides, name2)) != null) {
    ...
    private static void addToFirstInLastOut(BackStackRecord transaction, BackStackRecord.C0006Op op, SparseArray<FragmentContainerTransition> transitioningFragments, boolean isPop, boolean isOptimizedTransaction) {
        boolean setFirstOut;
        boolean setLastIn;
