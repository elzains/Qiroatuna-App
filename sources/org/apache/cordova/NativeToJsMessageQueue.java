...
public class NativeToJsMessageQueue {
    ...
    public void setBridgeMode(int value) {
        if (value < -1 || value >= this.bridgeModes.size()) {
            LOG.m21d(LOG_TAG, "Invalid NativeToJsBridgeMode: " + value);
            return;
        }
        BridgeMode newMode = value < 0 ? null : this.bridgeModes.get(value);
        if (newMode != this.activeBridgeMode) {
            LOG.m21d(LOG_TAG, "Set native->JS mode to " + (newMode == null ? "null" : newMode.getClass().getSimpleName()));
            synchronized (this) {
    ...
    public void addPluginResult(PluginResult result, String callbackId) {
        if (callbackId == null) {
            LOG.m25e(LOG_TAG, "Got plugin result with no callbackId", new Throwable());
            return;
        }
    ...
    private void enqueueMessage(org.apache.cordova.NativeToJsMessageQueue.JsMessage r3) {
        /*
            ...
            org.apache.cordova.LOG.m21d(r0, r1)     // Catch:{ all -> 0x001e }
    ...
    public void setPaused(boolean value) {
        if (this.paused && value) {
            LOG.m25e(LOG_TAG, "nested call to setPaused detected.", new Throwable());
        }
        this.paused = value;
        if (!value) {
            synchronized (this) {
    ...
    public static class LoadUrlBridgeMode extends BridgeMode {
        ...
        private final CordovaInterface f24cordova;
        ...
        public LoadUrlBridgeMode(CordovaWebViewEngine engine2, CordovaInterface cordova2) {
            this.engine = engine2;
            this.f24cordova = cordova2;
        }

        public void onNativeToJsMessageAvailable(final NativeToJsMessageQueue queue) {
            this.f24cordova.getActivity().runOnUiThread(new Runnable() {
                public void run() {
    ...
    public static class EvalBridgeMode extends BridgeMode {
        ...
        /* renamed from: cordova  reason: collision with root package name */
        private final CordovaInterface f23cordova;
        /* access modifiers changed from: private */
        public final CordovaWebViewEngine engine;

        public EvalBridgeMode(CordovaWebViewEngine engine2, CordovaInterface cordova2) {
            this.engine = engine2;
            this.f23cordova = cordova2;
        }

        public void onNativeToJsMessageAvailable(final NativeToJsMessageQueue queue) {
            this.f23cordova.getActivity().runOnUiThread(new Runnable() {
                public void run() {
                    String js = queue.popAndEncodeAsJs();
                    if (js != null) {
