...
public class Globalization extends CordovaPlugin {
    ...
    private JSONObject getDatePattern(JSONArray options) throws GlobalizationError {
        JSONObject obj = new JSONObject();
        try {
            SimpleDateFormat fmtDate = (SimpleDateFormat) DateFormat.getDateFormat(this.f21cordova.getActivity());
            SimpleDateFormat fmtTime = (SimpleDateFormat) DateFormat.getTimeFormat(this.f21cordova.getActivity());
            ...
            if (options.getJSONObject(0).has(OPTIONS)) {
                ...
                if (!innerOptions.isNull(FORMATLENGTH)) {
                    ...
                    if (fmtOpt.equalsIgnoreCase(MEDIUM)) {
                        fmtDate = (SimpleDateFormat) DateFormat.getMediumDateFormat(this.f21cordova.getActivity());
                    } else if (fmtOpt.equalsIgnoreCase(LONG) || fmtOpt.equalsIgnoreCase(FULL)) {
                        fmtDate = (SimpleDateFormat) DateFormat.getLongDateFormat(this.f21cordova.getActivity());
                    }
                }
    ...
    private JSONObject getDateNames(JSONArray options) throws GlobalizationError {
        ...
        try {
            ...
            for (String name2 : namesMap.keySet()) {
                namesList.add(name2);
            }
